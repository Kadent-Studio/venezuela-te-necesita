...
**Skills a usar al codear:** `interface-design` para el listado en grid de `/`;
`emil-design-eng` e `impeccable` para el pulido de UI/interacción; `caveman` como modo de
comunicación (eficiencia de tokens) durante la implementación.
...
- [ ] Instalar dependencias: `prisma`, `@prisma/client`, `zod`, `@clerk/nextjs`,
      ...
      `@vercel/blob`, y tipos (`@types/leaflet`).
- [ ] Provisionar **Neon Postgres** y **Vercel Blob** vía Vercel Marketplace; volcar
      variables con `vercel env pull` (`DATABASE_URL`, `BLOB_READ_WRITE_TOKEN`). Añadir
      `CLERK_*` y `IP_HASH_SECRET`.
...
- [ ] `POST /api/reports/upload`: token de *client upload* a **Vercel Blob** (solo
      `image/*`, límite de tamaño). Guardar `photoUrl` en el reporte.
- [ ] `/` (principal): listado en **grid** (skill `interface-design`) con **carga
      progresiva** (scroll infinito + "Cargar más"); `ReportCard` muestra la **foto** del
      lugar. CTA "Solicitar ayuda" abre **modal** con `ReportForm`, incl. **carga de foto**
      (en esta fase la ubicación puede ser entrada simple lat/lng + referencia; el mapa
      llega en Fase 2).
...
- [ ] `PATCH /api/admin/reports/[id]`: actualiza `verified` (+`verifiedBy/At`), `stage`,
      `discardReason`, `handledBy`; permite **retirar la foto** (`photoUrl = null`) como
      moderación. Devuelve `401` sin sesión.
